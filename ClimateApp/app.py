...
def precipitation():
    ...
    # Query and summarize daily precipitation across all stations for the (last year only) available data
    ...
    # Return a dict with the date as the key and the daily prcp total as value
    prcp_dates = []
    prcp_totals = []
...
def stations():
    ...
    # Returns the list of active weather stations in Hawaii
    select = [measurement.station]
    active_stations = session.query(*select).\
        group_by(measurement.station).all()
    session.close()

    # Return a dict with the date = key and the daily precipitation total = value
...
def tobs():
    ...
    select = [measurement.date, 
        measurement.tobs]
    station_temps = session.query(*select).\
    ...
    # Return a dict with the date=key and the daily temperature observation = value
    observation_dates = []
    temp_observations = []
...
def trip1(start_date, end_date='2017-08-23'):
    # Calculate minimum, average and maximum temperatures for the range of dates starting with start date. If no end date is provided, 2017-08-23 becomes the end date.
    ...
    # If the query returned non-null values return the results, return an error message if not
    if trip_dict['Min']: 
...
def trip2(start_date, end_date='2017-08-23'):
    ...
    # If no valid end date is provided, 2017-08-23 becomes the default date.
